...
{
   assert ( stuArray->arr != NULL );
   StudentType **newarray = calloc ( stuArray->capacity * 2, sizeof ( StudentArray ) );
   memcpy ( newarray, stuArray->arr, stuArray->capacity * sizeof ( StudentArray ) );
   free ( stuArray->arr );
...
void addStudent ( StudentArray * stuArray, StudentType * stu )
{
   ...
   assert ( stuArray->count < stuArray->capacity );
